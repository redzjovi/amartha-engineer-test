...
func (u *AuthUsecase) VerifyIsAdmin(ctx context.Context, request *model.AuthVerifyIsAdminRequest) (*model.Auth, error) {
	err := u.Validate.Struct(request)
	if err != nil {
		u.Log.Warnf("Invalid request body : %+v", err)
		return nil, fiber.ErrBadRequest
	...
	count, err := u.userRoleRepository.CountByUserIdAndRoleCode(tx, user.ID, entity.RoleCodeAdmin)
	if err != nil {
		u.Log.Warnf("Failed count user role by user id and role code : %+v", err)
		return nil, fiber.ErrInternalServerError
	} else if count == 0 {
		return nil, fiber.ErrNotFound
	}
